...
from model.objFileLoader import objObject
...
class Create_geom() :
    ...
    def cylinder():
        md = gl.MeshData.cylinder(rows=10, cols=20, radius=[1.0, 1.0], length=3.)


        geom = gl.GLMeshItem(meshdata=md, smooth=False, drawFaces=False, drawEdges=True, edgeColor=(1,1,0,1)) 
        return geom
        # draw cube
    ...
    def cube():
        ...
        geom = gl.GLMeshItem(vertexes=vertexes, faces=faces, faceColors=colors,
                            drawEdges=True)
        return geom
    ...
    def obj(fpath):
        ...
        geom = gl.GLMeshItem(vertexes=vertexes, faces=mfaces,
                            drawEdges=True)
        return geom
